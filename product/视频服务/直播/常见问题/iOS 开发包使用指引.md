...
4. 美颜滤镜、水印图标、切后台的垫片推流、背景音乐、耳返混响 等等一系列高级功能，推荐大家关注我们的 [官方文档](http://tce.fsphere.cn/document/product/454/7879)。
...
4. 截图、截视频（把视频一部分截取下来压成 MP4）等高级特性，可以参考完整的接入文档 [DOC](http://tce.fsphere.cn/document/product/454/7880#step-9.3A-.E6.88.AA.E6.B5.81.E5.BD.95.E5.88.B6.EF.BC.88.E4.BB.85.E7.9B.B4.E6.92.AD.EF.BC.89)。
...
- **实时通话**
 ...
 + 超级链路的 URL 拼装方式不同于普通的 CDN 播放地址，需要参考 [官方文档](http://tce.fsphere.cn/document/product/454/9849) 进行拼装。

- **多路混流**
 + 可以使用云平台端 [API](http://tce.fsphere.cn/document/product/454/9850) 指定将几条视频流混合起来，API 可以指定混流的各种参数，比如小画面的位置等等。
...
更多详细信息，推荐关注我们的 [官方文档](http://tce.fsphere.cn/document/product/584/9367)。


## 视频编辑
视频编辑器具有比较复杂的交互逻辑，这也决定了其 UI 复杂度很高，所以我们比较推荐复用 SDK 开发包中的 UI 源码， VideoEditor 目录包含短视频编辑器的 UI 代码。

如果要自己定制 UI 界面，推荐关注我们的 [官方文档](http://tce.fsphere.cn/document/product/584/9375)。


## 视频拼接
视频拼接器具有比较复杂的交互逻辑，这也决定了其 UI 复杂度很高，所以我们比较推荐复用 SDK 开发包中的 UI 源码， VideoJoiner 目录包含短视频拼接器的 UI 代码。

如果要自己定制 UI 界面，推荐关注我们的 [官方文档](http://tce.fsphere.cn/document/product/584/9370)。
