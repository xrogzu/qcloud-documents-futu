...
### 5. 文件预览
使用 [视频播放](http://tcecqpoc.fsphere.cn/document/product/584/9372) 即可预览刚才生成的 MP4 文件，需要在调用 startPlay 时指定播放类型为 [PLAY_TYPE_LOCAL_VIDEO](http://tcecqpoc.fsphere.cn/document/product/584/9372#step-3.3A-.E5.90.AF.E5.8A.A8.E6.92.AD.E6.94.BE6) 。

### 6. 获取签名
要把刚才生成的 MP4 文件发布到云平台上，App 需要拿到上传文件用的短期有效上传签名，这部分有独立的文档介绍，详情请参考 [Server端集成 - 签名派发](http://tcecqpoc.fsphere.cn/document/product/584/9371)。
...
### 8.发布结果
通过 [错误码表](http://tcecqpoc.fsphere.cn/document/product/584/10176) 来确认短视频发布的结果。
