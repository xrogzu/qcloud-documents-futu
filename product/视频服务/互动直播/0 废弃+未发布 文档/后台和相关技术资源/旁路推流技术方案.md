...
由于互动直播采用了云平台直播服务，所以在使用旁路推流功能之前要先申请开通云平台直播服务，审批通过后即可在[管理控制台](http://console.tcecqpoc.fsphere.cn/live)进行直播频道管理、统计分析以及设置等操作。除了手工操作之外，用户也可以通过[直播服务API](http://tcecqpoc.fsphere.cn/wiki/直播API)所提供的能力在业务后台进行直播管理操作。
