...
The Demo project for multi-person messaging is created through Eclipse.
...
Run-time flow charts and API documents are located under the doc directory.
...
SDK libraries are provided in the form of jar files and dynamic link libraries (so). Place the so files under <project name>/libs/armeabi and place eup_1.9.2__allproguad_rqdp.jar, imsdk.jar and qavsdk.jar under /libs. Refreshing Eclipse can add the project reference automatically.
...
SDK requires permissions for camera, Bluetooth earphone, audio recording, network access, SD card read/write, incoming call monitoring and so on.
