## 互动直播录制开发
录制的文件将存储在云平台提供的点播服务上，用户可通过点播的管理控制台、API进行管理、转码、分发等操作。<br/><br/>
**使用录制功能前，请先在控制台开通云平台点播服务，否则将无法使用**。
...
Android录制功能的详细实现见[新随心播](http://github.com/zhaoyang21cn/ILiveSDK_Android_Demos)
...
IOS录制功能的详细实现见[新随心播](http://github.com/zhaoyang21cn/ILiveSDK_iOS_Demos)

### 2 视频管理

通过音视频通信SDK录制的视频将存储在点播服务中

1. 用户访问点播的[管理控制台](http://console.tcecqpoc.fsphere.cn/video)可以对录制的文件进行相应的管理操作<br/>
2. 用户也可以通过点播提供的API进行管理操作，[API手册](http://tcecqpoc.fsphere.cn/document/product/266/7788)<br/>
3. DescribeVodPlayInfo能根据文件名（开始录制Api中，录制参数所填文件名）获取到录制
文件下载地址。详见[参考文档](http://tcecqpoc.fsphere.cn/document/product/266/8586)

### 3 价格和计费说明

按照录制月并发最高路数收费，价格：30元/路/月。
此外，录制功能会使用点播服务的能力，在云点播中会产生存储、流量的费用。[计费规则](http://tcecqpoc.fsphere.cn/doc/product/268/5129#2..E5.BD.95.E5.88.B6.E7.9B.B8.E5.85.B3.E8.AE.A1.E8.B4.B9)：
