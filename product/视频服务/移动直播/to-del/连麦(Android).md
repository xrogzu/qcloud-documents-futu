本文档主要介绍云平台连麦功能的对接方案，如果您想要了解连麦的原理，欢迎阅读 [如何实现连麦功能？](http://tcecqpoc.fsphere.cn/document/product/454/8092)

## 更新版本
RTMP SDK 1.8.2 开始才支持连麦功能，请到 [下载页](http://tcecqpoc.fsphere.cn/document/product/454/7873) 更新最新版本的 RTMP SDK。同时，我们的 小直播 [DEMO](http://tcecqpoc.fsphere.cn/document/product/454/6991) 也已经集成了这套解决方案的示例代码。
...
## 同房连麦
### step1. “大主播”推流
我们在 [Android 推流](http://tcecqpoc.fsphere.cn/document/product/454/7885) 中有详细介绍如何在主播端开启直播功能，这里您可以直接参考，流程上都是一样的。所以，如果您是第一次接触RTMP SDK，务必要先阅读一下基础推流功能的文档。
...
- **1.1 推流URL加连麦参数**
在[如何获取推流地址](http://tcecqpoc.fsphere.cn/document/product/454/7915#.E5.90.8E.E5.8F.B0.E8.87.AA.E5.8A.A8.E6.8B.BC.E8.A3.85.EF.BC.9F)中，我们详细介绍了推流地址的拼装规则，如果要做连麦，推流地址里面还要额外加一段参数：
...
在实现上，这里可以采用 C2C（Client To Client）消息通道，云平台 IM 通讯服务提供了 C2C 解决方案，您可以参考 [如何搭建聊天室](http://tcecqpoc.fsphere.cn/document/product/454/7980) 了解 IM 服务的使用方案。
...
我们在 [Android播放](http://tcecqpoc.fsphere.cn/document/product/454/7886) 中有详细介绍如何在观众端使用播放功能，如果主播们之间也像普通观众一样，采用 CDN 分发的[播放地址](http://tcecqpoc.fsphere.cn/document/product/454/7915#.E5.90.8E.E5.8F.B0.E8.87.AA.E5.8A.A8.E6.8B.BC.E8.A3.85.EF.BC.9F)，当然是可以看到画面听到声音的。
...
![](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/59c492abef77cddaf026cfd7509de678/image.png)
 ...
 + 播放地址必须要加防盗链签名，签名方法参考 [推流防盗链的计算](http://tcecqpoc.fsphere.cn/document/product/454/7915#.E9.98.B2.E7.9B.97.E9.93.BE.E7.9A.84.E8.AE.A1.E7.AE.97.EF.BC.9F)。因为几乎所有云平台的客户都配置了推流防盗链KEY，为了减少您的接入成本，可以直接使用推流防盗链KEY。
...
### step5. 多路混流
- **跨房连麦**：目前不支持服务端混流，只支持客户端混流。我们会在春节后启用新的commen cgi 风格接口，届时大家会在 [云平台服务端API](http://tcecqpoc.fsphere.cn/document/product/454/7920) 中查到该接口的定义。
