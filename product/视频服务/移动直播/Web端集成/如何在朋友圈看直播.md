...
小直播源码集中包含 Web 分享页面的 [DEMO源码](/document/product/454/6991) ，实现了**直播观看**、**文本消息**以及**点赞**等功能。
...
| 配置项 | 含义 | 参考文档 |
|:-------: |:--------------|:----------:|
| SERVER | 为该网页提供视频播放信息的业务服务器。 | [DOC](/document/product/454/8046#step4.3A-web.E5.90.8E.E5.8F.B0.E6.90.AD.E5.BB.BA) |
| accountMode | IM SDK 的账号集成模式（配置要跟小直播中保持一致）。 | [DOC](/document/product/454/7980) | 
| sdkAppID | IM 服务开通后分配的一个id，如果分享URL的参数中携带可以不配置。 | [DOC](/document/product/454/7953#3.2-im-sdk-appid) |
| accountType |  IM 服务开通后分配的一个type，如果分享URL的参数中携带可以不配置。 | [DOC](/document/product/454/7953#3.3-im-sdk-.E8.B4.A6.E5.8F.B7.E7.B1.BB.E5.9E.8B) |
...
- 静态网页的部署推荐使用[CDN 内容分发网络](/product/cdn)，CDN 的优势就是能极大的缩减用户打开页面的速度，从而提升用户体验。
...
这些信息就需要 Web 页面中的 xzb.js 通过 ajax 异步请求到您的业务后台服务器查询。我们在小直播中，将这一查询协议定义为 [GetUserInfo](/document/product/454/7895#9..E8.8E.B7.E5.8F.96.E6.8C.87.E5.AE.9A.E4.B8.BB.E6.92.AD.E7.9A.84.E8.AF.A6.E7.BB.86.E4.BF.A1.E6.81.AF)。

小直播源码集中包含 PHP 后台服务器 [DEMO源码](/document/product/454/6991) ，其中 GetUserInfo.php 中提供了对 GetUserInfo 协议的实现。
...
| 参数名 | 含义 | 备注 |
|---------|---------|---------|
| sdkappi | IM 服务开通后分配的一个id，用来进入聊天室的必备信息。 | [参考文档](/document/product/454/7953#3.2-im-sdk-appid) |
| acctype |  IM 服务开通后分配的一个type，跟 sdkappi 配合使用。 | [参考文档](/document/product/454/7953#3.3-im-sdk-.E8.B4.A6.E5.8F.B7.E7.B1.BB.E5.9E.8B) |
...
[怎么去掉“非微信官方网页，将由微信转换为手机预览模式”的提示页面？](http://www.zhihu.com/question/45748989)
