...
- **获取开发包**
  [下载](/document/product/454/7873#Windows) SDK 开发包，并按照[工程配置](/document/product/454/13625)指引将 SDK 嵌入您的 APP 开发工程。

- **推流测试URL**
  Demo支持自动从后台获取到推流测试 URL，点击上图中的 **New** 圆形按钮即可获得推流地址。

  另外还有一种手动从云平台后台的控制台生成的方式，[开通](http://console.tcecqpoc.fsphere.cn/live)直播服务后，可以使用 [直播控制台>>直播码接入>>推流生成器](http://console.tcecqpoc.fsphere.cn/live/livecodemanage) 生成推流地址，详细信息可以参考 [获得推流播放URL](/document/product/454/7915)。
...
本地摄像头的画面镜像、静音、清晰度设置等功能，可以参考[接口列表](/document/product/454/13626)，获取更多详细信息。
...
可以参考 [接口列表](/document/product/454/13626)，获取更多详细信息。
