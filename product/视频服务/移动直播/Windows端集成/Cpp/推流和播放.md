...
下载 [Windows SDK+Demo](http://tcecqpoc.fsphere.cn/document/product/454/7873#Windows) ，下载 C++ 版本的 zip 包，解压后打开 LiteAVDemo(Qt).exe 程序，首页包含3种模式，**推流+拉流**、**直播体验室**、**双人音视频**和**多人音视频**等功能。

选择**推流+拉流**后，您仅需要点击 **New** 按钮即可以获取一个新的测试推流地址。开启推流及播放后，Demo 效果如下：

![](http://imgcache.tcecqpoc.fsphere.cn/image/main.qcloudimg.com/raw/3c33d57e5aae6f4ede41140b5ca833e9.png)
...
- step2：进入[直播控制台>>直播码接入>>接入配置](http://console.tcecqpoc.fsphere.cn/live/livecodemanage) 配置推流防盗链KEY 和 API鉴权KEY， 配置方法可参考 [DOC](http://tcecqpoc.fsphere.cn/document/product/454/7953#1.2-.E7.9B.B4.E6.92.AD.E7.A0.81.E6.8E.A5.E5.85.A5.E6.80.8E.E4.B9.88.E9.85.8D.EF.BC.9F)。最终这些 URL 都是由您的后台业务服务器自动生成的。

- step3：进入[直播控制台>>直播码接入>>推流生成器](http://console.tcecqpoc.fsphere.cn/live/livecodemanage) 可以生成测试用的 [推流URL](http://tcecqpoc.fsphere.cn/document/product/454/7915) 。播放URL的拼接跟推流URL一样简单，只是需要把子域名从 **livepush** 改成 **liveplay**。

- step4：[下载](http://tcecqpoc.fsphere.cn/document/product/454/7873#Windows) SDK 开发包，并按照 [工程配置](http://tcecqpoc.fsphere.cn/document/product/454/13669) 将 SDK 嵌入您的 APP 开发工程。之后参考本文档的**代码对接**进行推流及播放的相关设置与接入。

- step5：您也可以在 [Windows SDK+Demo](http://tcecqpoc.fsphere.cn/document/product/454/7873#MiniPrograms) 里下载 Demo 的源码进行参考。
...
创建完成后，在开始推流之前，可以调用 TXLivePusher 的相关 API 接口，设置镜像效果、码率、分辨率和填充模式等。具体 API 可参考 [接口列表](http://tcecqpoc.fsphere.cn/document/product/454/13671)。
...
TXLivePusher 的 enumCameras 函数可以获取摄像头的索引值。具体示例可参考 [接口列表](http://tcecqpoc.fsphere.cn/document/product/454/13671)。
...
本地摄像头的画面镜像、静音、清晰度设置等功能，可以参考 [接口列表](http://tcecqpoc.fsphere.cn/document/product/454/13671)，获取更多详细信息。
...
可以参考 [接口列表](http://tcecqpoc.fsphere.cn/document/product/454/13671)，获取更多详细信息。
