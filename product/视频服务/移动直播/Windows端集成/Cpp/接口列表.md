...
在云平台官网 [下载](http://tcecqpoc.fsphere.cn/document/product/454/7873#Windows) SDK 开发包，可于SDK\include中可以看到详细的接口注释。
...
| rendHwnd | HWND         | 承载预览画面的 HWND                             |
| rect     | const RECT & | 指定视频图像在 HWND 上的渲染区域                      |
| userType | enum         | 如果srcType = TXE_VidoeSrc_User_Data，指定用户数据的视频格式，参考TXEVideoUserDataSrcType定义。 |
...
| 参数   | 类型           | 说明                                       |
| ---- | ------------ | ---------------------------------------- |
| url  | const char * | 一个合法的推流地址，支持 rtmp 协议（URL 以 “rtmp://” 打头 ，云平台推流 URL 的获取方法见 [DOC]( http://tcecqpoc.fsphere.cn/document/product/454/7915 "云平台-推拉流地址Doc") ） |
...
| 参数   | 类型           | 说明                                       |
| ---- | ------------ | ---------------------------------------- |
| url  | const char * | 一个合法的拉流地址，视频播放 URL（URL 以 “rtmp://” 打头 ，云平台拉流 URL 的获取方法见 [DOC]( http://tcecqpoc.fsphere.cn/document/product/454/7915 "云平台-推拉流地址Doc") ） |
...
```
	...
	TXE_AudioSrc_User_PCM = 1001,				音频采集源来自用户传入的PCM裸音频的数据
	TXE_AudioSrc_User_AAC = 1002,				音频采集源来自用户传入AAC音频编码数据
```
...
```
	...
	TXE_VidoeSrc_User_Data = 1001,          视频采集源来自用户传入的视频数据，
```

#### TXEVideoUserDataSrcType

用户导入的视频数据仅提供几种比较常见的场景编码参数,TXEVideoCaptureSrcType == TXE_VidoeSrc_User_Data时，此参数生效。

```
	TXE_UserData_Undefined = 0,		  屏蔽用户输入的所有视频数据，
	TXE_UserData_Yuv420P,			  TXE_VidoeSrc_User_Data传入的数据是Yuv420P的视频数据
	TXE_UserData_H264Nal,			  TXE_VidoeSrc_User_Data传入的数据是H264Nal的视频编码数据
```
