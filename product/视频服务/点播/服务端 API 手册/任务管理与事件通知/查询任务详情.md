...
```
http://vod.api.qcloud.com/v2/index.php?Action=GetTaskInfo
...
| message | String | 错误信息。 |
| type | String | 任务类型，目前有 Procedure（任务流），transcode（转码），clipVideo（剪辑）。 |
| status   | String | 任务状态，有WAITING/PROCESSING/FINISH 三种。               |
| createTime | Integer | 任务创建时间（Unix时间戳）。 |
| beginProcessTime | Integer | 任务开始执行的时间（Unix时间戳），若任务还在排队中，该值为0。 |
| finishTime | Integer | 任务信息最近更新时间（Unix时间戳），若任务还未完成，该值为0。 |
| data | Object | 任务详细信息，type的值不同，data数据也不相同，当status为WAITING，该字段不存在。 |
...
当type为clipVideo，data内容与[事件通知-剪辑完成](/document/product/266/10157)回调包体中data字段相同
