...
<iframe src="http://playvideo.qcloud.com/vod/4564972818712033224/iplayer.html?appid=1253131631&fileid=4564972818712033224&autoplay=0&sw=1280&sh=720" frameborder="0" width="70%" height="360" scrolling="no"></iframe> <!-- 页面内多处使用iframe代码，后面的JS代码只需要使用一次(作用是调整iframe的高度) --> 
...
- **动态水印**：使用[APNG](http://zh.wikipedia.org/wiki/APNG)动图作为水印，该动画图片位于视频的固定位置循环播放。
...
## 视频鉴黄
在个人直播录制、UGC 短视频等场景中，视频内容是不可预知的。为了避免一些违规内容出现在点播平台上，开发者会要求先对上传的视频做审核，确认合规后再进行转码和分发。云点播支持对视频进行 AI 鉴黄，自动识别视频是否涉及色情内容。

### 使用 AI 鉴黄
AI 鉴黄功能需要集成在[视频处理任务流](http://tcecqpoc.fsphere.cn/document/product/266/11700#.E4.BB.BB.E5.8A.A1.E6.B5.81)中使用，检查结果通过[事件通知](http://tcecqpoc.fsphere.cn/document/product/266/7829#.E4.BA.8B.E4.BB.B6.E9.80.9A.E7.9F.A5.E7.AE.80.E4.BB.8B)的方式来通知开发者。

云点播内置了一个任务流 [`QCVB_ProcessUGCFile`](/document/product/266/11700#qcvb_processugcfile) 用于 UGC 短视频鉴黄场景，当用户使用该任务流并指定进行 AI 鉴黄时，鉴黄操作将优先执行，并根据鉴黄结果来决定是否进行后续处理（转码、打水印和截图等）。流程图如下：

![AI 鉴黄流程](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/74b9863756590f00ac72b303dd750455/image.jpg)
...
op2=>operation: AI 鉴黄
op3=>operation: 人工审核
op4=>operation: 转码/打水印/截图
cond1=>condition: 涉黄？
cond2=>condition: 终止任务流？
...
### AI 鉴黄配合人工审核
AI 鉴黄的辨别结果可能并不能完全符合开发者的要求，对于被判定为涉黄的视频，开发者可能会希望通过人工审核的方式来再次确认。

启用人工审核时，开发者首先使用任务流 [`QCVB_ProcessUGCFile`](/document/product/266/11700#qcvb_processugcfile) 进行 AI 鉴黄，其中鉴黄模版选择 10。这样当视频涉黄时，任务流就会终止，同时涉黄的截图 URL 会通过[事件通知](http://tcecqpoc.fsphere.cn/document/product/266/7829#.E4.BA.8B.E4.BB.B6.E9.80.9A.E7.9F.A5.E7.AE.80.E4.BB.8B)的方式通知到开发者。开发者人工审核截图，如果确实涉黄，可以[删除视频](http://tcecqpoc.fsphere.cn/document/product/266/7838)；如果确认不涉黄，那么可以重新发起任务流来进行转码等处理（此时 `QCVB_ProcessUGCFile` 的 AI 鉴黄模版参数填 0 即可，表示不需鉴黄）。流程图如下：

![人工审核](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/bdee2c9c91cb62734d3a06aa645eec47/image.jpg)

<!--
```flow
st=>start: 鉴黄完成事件通知
e=>end: 结束
op1=>operation: 等待视频处理完成
op2=>operation: 人工审核
op3=>operation: 删除视频
op4=>operation: 重新发起任务流
cond1=>condition: AI 鉴黄结果为涉黄？
cond2=>condition: 人工审核结果为涉黄？
...
<table>
    ...
    <tr>
        ...
        <td>
            终止任务流（转码、打水印、截图等均不会执行）
        </td>
        <td rowspan=2>    
            时长小于 500 秒的视频，每 1 秒采样 1 次；</br>时长大于或等于 500 秒的视频，每 1% 时长采样 1 次。
        </td>
    </tr>
    <tr>
        <td>
