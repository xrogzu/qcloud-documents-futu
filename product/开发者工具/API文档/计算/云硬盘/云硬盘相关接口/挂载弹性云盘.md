...
使用限制：
1. 只支持弹性云盘。详见[DescribeCbsStorages（查询云硬盘信息）](http://tce.fsphere.cn/doc/api/364/2519)接口输出参数中portable字段解释
1. 云硬盘必须处于非挂载状态，且状态为normal。详见[DescribeCbsStorages（查询云硬盘信息）](http://tce.fsphere.cn/doc/api/364/2519)接口输出参数中storageStatus、attached字段解释
1. 单个云主机最多可挂载云硬盘限制见[DescribeInstancesCbsNum（查询云主机已挂载弹性云盘数量）](http://tce.fsphere.cn/doc/api/364/2528)输出参数说明
