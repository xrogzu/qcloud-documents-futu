...
使用限制：
1. 只支持扩容弹性云盘。详见[DescribeCbsStorages（查询云硬盘信息）](/doc/api/364/2519)接口回包中portable字段解释。随云主机创建的云硬盘需通过[ResizeInstance（调整实例配置（包年包月）)](/doc/api/229/1306) 或 [ResizeInstanceHour（调整实例配置（按量计费））](/doc/api/229/1344) 接口扩容
