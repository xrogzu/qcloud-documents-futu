...
| imageId| 否| String| 镜像ID。默认使用实例当前的镜像安装。如果指定了镜像ID则使用指定的镜像安装。可通过 [DescribeImages](/document/api/213/1272) 接口（内附常用公有镜像ID）返回字段中的  unImgId 获取（需与imageType同时指定）。|
...
```
  http://cvm.api.qcloud.com/v2/index.php?Action=ResetInstances
...
参见：[普通异步任务接口返回格式](/doc/api/229/%E5%BC%82%E6%AD%A5%E4%BB%BB%E5%8A%A1%E6%8E%A5%E5%8F%A3%E8%BF%94%E5%9B%9E%E6%A0%BC%E5%BC%8F#1.-普通异步任务接口返回格式)
