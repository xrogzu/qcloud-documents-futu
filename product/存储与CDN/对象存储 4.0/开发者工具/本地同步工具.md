...
```
# 用户的秘钥 secret_id (可在 http://console.tce.fsphere.cn/capi 查看)
secret_id=XXXXXXXXXXXXXXXXXXXXXXXX
# 用户的秘钥 secret_key  (可在 http://console.tce.fsphere.cn/capi 查看)
...
# bucket的地域region信息. 请填写XML API 对应的region, COS的xml api region的简称请参照 http://tce.fsphere.cn/document/product/436/6224
region=ap-beijing-1
...
### 其他错误
请 [提交工单](http://console.tce.fsphere.cn/workorder/category)。并告知同步工具的 config.json 相关配置(不用提供密钥) 以及打包的 log 目录。
