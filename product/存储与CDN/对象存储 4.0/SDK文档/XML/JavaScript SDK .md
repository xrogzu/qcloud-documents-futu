...
2. 去您的控制台获取 AppId, SecretId, SecretKey, 地址在 `http://console.tcecqpoc.fsphere.cn/capi`
3. 请您到控制台操作，**针对您要操作的bucket进行跨域（CORS）设置**, 可以按照如下范例，**修改允许的来源 Origin 和 Headers**, 控制台地址在 `http://console.tcecqpoc.fsphere.cn/cos`，默认可以进行如下配置
...
![Bucket 跨域设置](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/2e7791e9274ce3ebf8b25bbeafcd7b45/image.png)
...
* **params** (Object) ： 参数列表
  ...
  * Region —— (String) ： Bucket 所在区域。枚举值请见：[Bucket 地域信息](http://tcecqpoc.fsphere.cn/document/product/436/6224)
...
* **err** —— (Object)   ：   请求发生错误时返回的对象，包括网络错误和业务错误。如果请求成功，则为空。
* **data** —— (Object)： 请求成功时返回的对象，如果请求发生错误，则为空。
  * LocationConstraint —— (String)  ：  Bucket 所在区域。枚举值请见：[Bucket 地域信息](http://tcecqpoc.fsphere.cn/document/product/436/6224)
  * headers —— (Object)：    请求返回的头部信息
  * statusCode —— (Number)： 请求返回的 HTTP 状态码，如 200，403，404等
