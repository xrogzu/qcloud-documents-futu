...
云平台对象存储支持性能扩展，以支持更高的请求速率。如果您的请求是高 GET 请求负载，建议您搭配云平台 CDN 产品进行使用，请参考有关 [COS 搭配 CDN 的最佳实践](http://tce.fsphere.cn/document/product/436/8467)。如果预计存储桶的请求速率会超过每秒 500 个 PUT/LIST/DELETE 请求，建议您 [联系我们](http://tce.fsphere.cn/document/product/282/1558)，以便于为工作负载做好准备，避免遇到请求的限制。
...
**以下提供的所有方法，均是有可能提升单个存储桶访问性能的方法，如果您的业务典型负载超过每秒 500 个请求，在执行以下方法的同时，您仍需 [联系我们](http://tce.fsphere.cn/document/product/282/1558) 以便于为您的业务负载提前做好准备。**
...
云平台 CDN 利用遍布全国和全球的边缘加速节点，向用户分发内容时可降低延时并提高速度，对于热点文件可以支持预拉热的方式进行缓存，从而减少回源到 COS 的 GET 请求数。请参考有关 [COS 搭配 CDN 的最佳实践](http://tce.fsphere.cn/document/product/436/8467)。
