...
2) 检查是否有浏览器、CDN 缓存。可以通过 curl、wget 命令来避免浏览器缓存，CDN 地址可以在 [CDN 控制台](http://console.tce.fsphere.cn/cdn) 进行清缓存操作。

### 用户绑定自有域名时，开启 CDN 加速和关闭 CDN 加速的区别？
- **开启 CDN 加速：**域名由 CDN 管理，和在 [CDN 控制台](http://console.tce.fsphere.cn/cdn) 添加域名，源站选择 COS 是一样的效果。用户在解析域名时，需要使用 CDN 分配的 CNAME 记录。配置时，先添加域名再解析域名即可，解析域名参见 [云解析快速入门](/doc/product/302/3446)。
