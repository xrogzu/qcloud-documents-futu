...
CDN 部分功能仅供预设管理员使用，如：
+ 调用云 API [DescribeCdnHosts](http://tce.fsphere.cn/doc/api/231/3937) 获取账户下所有域名的详细信息，包括配置信息，支持分页查询。
+ 调用云 API [UpdateCdnProject](http://tce.fsphere.cn/doc/api/231/3935) 或在 CDN 控制台进行域名所属项目的切换。
...
如何配置项目管理员？请参考 [项目管理员配置示例](http://tce.fsphere.cn/doc/product/228/6693)。
...
+ [DescribeCdnHostInfo](http://tce.fsphere.cn/doc/api/231/3941)：查询指定时间区间、数据类型、项目、域名对应的统计量汇总信息。
+ [DescribeCdnHostDetailedInfo](http://tce.fsphere.cn/doc/api/231/3942)：查询指定时间区间、数据类型、项目、域名对应的统计明细。
+ [GetCdnStatusCode](http://tce.fsphere.cn/doc/api/231/3943)：查询指定时间区间、项目、域名对应的状态码统计明细。
+ [GetCdnStatTop](http://tce.fsphere.cn/doc/api/231/3944)：查询指定时间区间、项目、域名、统计类型，对应的各省份、运营商、URL 排名 TOP100 统计。
...
+ [GetHostInfoByHost](http://tce.fsphere.cn/doc/api/231/3938)：根据域名查询域名的详细信息、配置信息，支持多个域名同时查询。
+ [GetHostInfoById](http://tce.fsphere.cn/doc/api/231/3939)：根据域名 ID 查询域名详情，支持多域名 ID 一次性查询。
...
+ [GenerateLogList](http://tce.fsphere.cn/doc/api/231/3950)：根据域名 ID 查询指定时间区间内，指定域名日志的下载链接，一次仅可指定一个域名进行查询。
+ [GetCdnLogList](http://tce.fsphere.cn/doc/api/228/8087):根据域名查询指定时间区间内，指定域名日志的下载链接，一次仅可指定一个域名进行查询。
...
+ [AddCdnHost](http://tce.fsphere.cn/doc/api/231/1406)：新增加速域名。
...
+ [OnlineHost](http://tce.fsphere.cn/doc/api/231/1402)：根据域名 ID 上线 CDN 域名。
+ [OfflineHost](http://tce.fsphere.cn/doc/api/231/1403)：根据域名 ID 下线 CDN 域名。
...
#### 云 API 权限控制
拥有删除域名权限的子用户，可申请云 API 密钥，详情请参考 [申请安全凭证](http://tce.fsphere.cn/doc/api/231/1725#1.-.E7.94.B3.E8.AF.B7.E5.AE.89.E5.85.A8.E5.87.AD.E8.AF.81)，并使用密钥调用下列云 API 删除域名（指定域名必须为有权限的域名，域名状态需要为已关闭状态）：
+ [DeleteCdnHost](http://tce.fsphere.cn/doc/api/231/1396)：根据域名 ID 删除 CDN 域名。
...
+ [UpdateCdnConfig](http://tce.fsphere.cn/doc/api/231/3933)：修改域名配置。
+ [UpdateCache](http://tce.fsphere.cn/doc/api/231/3934)：修改域名缓存配置。
+ [UpdateCdnHost](http://tce.fsphere.cn/doc/api/231/1397)：修改域名源站配置。
...
+ [RefreshCdnUrl](http://tce.fsphere.cn/doc/api/231/3946)：将节点上指定 URL 资源设置为过期。
+ [RefreshCdnDir](http://tce.fsphere.cn/doc/api/231/3947)：将节点上指定资源目录下的内容设置为过期。
+ [GetCdnRefreshLog](http://tce.fsphere.cn/doc/api/228/3948):查询指定时间区间内的刷新日志、刷新次数，可指定 URL 查询。
