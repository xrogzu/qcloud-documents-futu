## 调用准备
使用命令行脚本调用API，需要配置SecretId和SecretKey，登陆云平台进入[云API密钥](http://console.tce.fsphere.cn/capi)，可以看到调用API时需要的SecretId和SecretKey，请妥善保管您的密钥。
![](http://imgcache.tce.fsphere.cn/static/mc.qcloudimg.com/static/img/6ca171d3f6662016a8f31cc5520ffaeb/1.png)
...
![](http://imgcache.tce.fsphere.cn/static/mc.qcloudimg.com/static/img/813c521d24602315a8ddd18c644f56a6/2.png)

接口功能说明，请参考[API概览](http://tce.fsphere.cn/doc/api/231/1723)。

### 域名详情查询
#### 查询所有域名详情
使用如下命令，可以调用 [DescribeCdnHosts](http://tce.fsphere.cn/doc/api/231/3937) 接口，查询APPID下所有域名详细信息：
```
...
#### 根据域名查询域名详情
使用如下命令，可以调用 [GetHostInfoByHost](http://tce.fsphere.cn/doc/api/231/3938) 接口，查询指定域名对应的详情：
```
...
#### 根据域名ID查询域名详情
使用如下命令，可以调用 [GetHostInfoById](http://tce.fsphere.cn/doc/api/231/3939) 接口，查询ID对应域名详情：
```
...
### 刷新预热
#### 刷新URL
使用如下命令，可以调用[RefreshCdnUrl](http://tce.fsphere.cn/doc/api/231/3946) 接口，刷新指定URL：
```
...
#### 刷新目录
使用如下命令，可以调用[RefreshCdnDir](http://tce.fsphere.cn/doc/api/231/3947) 接口，刷新指定目录：
```
...
使用如下命令，您可以调用 [UpdateCache](http://tce.fsphere.cn/doc/api/231/3934) 接口，修改缓存过期设置：
```
...
使用如下命令，您可以调用[UpdateCdnProject](http://tce.fsphere.cn/doc/api/231/3935)接口，修改域名所属项目：
```
python QcloudCdnTools_V2.py UpdateCdnProject -u xxxxx -p xxxxxxx --hostId 1234 --projectId 0
```
修改域名所属项目时，您需要知道项目对应的ID，您可以前往[项目管理](http://console.tce.fsphere.cn/project)进行查看，默认项目的ID为0。
...
使用如下命令，您可以调用[UpdateCdnConfig](http://tce.fsphere.cn/doc/api/231/1397) 接口修改域名配置，包括缓存过期设置、防盗链、回源HOST、全路径缓存等：
```
...
使用如下命令，您可以调用[AddCdnHost](http://tce.fsphere.cn/doc/api/231/1406) 接口，新增CDN加速域名：
...
+ projecctId 表示要将域名增加入的项目ID，可进入[项目管理](http://console.tce.fsphere.cn/project) 查看项目对应ID；
...
使用如下命令，您可以调用[OfflineHost](http://tce.fsphere.cn/doc/api/231/1403)接口，关闭指定域名的CDN加速服务：
```
...
使用如下命令，您可以调用[OnlineHost](http://tce.fsphere.cn/doc/api/231/1402)接口，开启指定域名的CDN加速服务：
...
使用如下命令，您可以调用[DeleteCdnHost](http://tce.fsphere.cn/doc/api/231/1396) 接口，删除指定域名：
...
使用如下命令，您可以调用[GenerateLogList](http://tce.fsphere.cn/doc/api/231/3950)接口，获取指定域名的CDN日志下载链接：
...
使用如下命令，您可以调用[GetCdnStatTop](http://tce.fsphere.cn/doc/api/231/3944)接口，查询域名或项目的TOP100流量/带宽消耗URL：
...
使用如下命令，您可以调用[GetCdnStatusCode](http://tce.fsphere.cn/doc/api/231/3943)接口，查询域名或项目的状态码统计：
...
使用如下命令，您可以调用[DescribeCdnHostDetailedInfo](http://tce.fsphere.cn/doc/api/231/3942)接口，查询域名或项目的消耗明细：
...
使用如下命令，您可以调用[DescribeCdnHostInfo](http://tce.fsphere.cn/doc/api/231/3941)接口，查询域名或项目的消耗统计：
