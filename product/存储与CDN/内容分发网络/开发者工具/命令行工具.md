...
接口功能说明，请参考[API概览](/doc/api/231/1723)。

### 域名详情查询
#### 查询所有域名详情
使用如下命令，可以调用 [DescribeCdnHosts](/doc/api/231/3937) 接口，查询APPID下所有域名详细信息：
```
...
#### 根据域名查询域名详情
使用如下命令，可以调用 [GetHostInfoByHost](/doc/api/231/3938) 接口，查询指定域名对应的详情：
```
...
#### 根据域名ID查询域名详情
使用如下命令，可以调用 [GetHostInfoById](/doc/api/231/3939) 接口，查询ID对应域名详情：
```
...
### 刷新预热
#### 刷新URL
使用如下命令，可以调用[RefreshCdnUrl](/doc/api/231/3946) 接口，刷新指定URL：
```
...
#### 刷新目录
使用如下命令，可以调用[RefreshCdnDir](/doc/api/231/3947) 接口，刷新指定目录：
```
...
使用如下命令，您可以调用 [UpdateCache](/doc/api/231/3934) 接口，修改缓存过期设置：
```
...
使用如下命令，您可以调用[UpdateCdnProject](/doc/api/231/3935)接口，修改域名所属项目：
```
...
使用如下命令，您可以调用[UpdateCdnConfig](/doc/api/231/1397) 接口修改域名配置，包括缓存过期设置、防盗链、回源HOST、全路径缓存等：
```
...
使用如下命令，您可以调用[AddCdnHost](/doc/api/231/1406) 接口，新增CDN加速域名：
...
使用如下命令，您可以调用[OfflineHost](/doc/api/231/1403)接口，关闭指定域名的CDN加速服务：
```
...
使用如下命令，您可以调用[OnlineHost](/doc/api/231/1402)接口，开启指定域名的CDN加速服务：
...
使用如下命令，您可以调用[DeleteCdnHost](/doc/api/231/1396) 接口，删除指定域名：
...
使用如下命令，您可以调用[GenerateLogList](/doc/api/231/3950)接口，获取指定域名的CDN日志下载链接：
...
使用如下命令，您可以调用[GetCdnStatTop](/doc/api/231/3944)接口，查询域名或项目的TOP100流量/带宽消耗URL：
...
使用如下命令，您可以调用[GetCdnStatusCode](/doc/api/231/3943)接口，查询域名或项目的状态码统计：
...
使用如下命令，您可以调用[DescribeCdnHostDetailedInfo](/doc/api/231/3942)接口，查询域名或项目的消耗明细：
...
使用如下命令，您可以调用[DescribeCdnHostInfo](/doc/api/231/3941)接口，查询域名或项目的消耗统计：
