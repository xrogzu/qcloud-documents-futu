...
### 查看系统分配域名
登录 [万象优图控制台](http://console.tcecqpoc.fsphere.cn/ci) ，单击【 Bucket 管理】进入管理页面，选择【域名管理】页签，即可看到系统分配域名。
![域名管理](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/ef6c7b9e2b7f896f738756c6f05f9685/image.png)

**系统分配域名- 内网跨区域访问：** 系统分配域名可以用于在同区域的不同云平台产品之间进行访问。但是，若需要实现内网跨区域的访问， 例如，所在区域为广州的 CVM 需要使用新加坡的万象优图上的数据，则需要使用 VPC 部署专属网络通道，实现高速访问体验。 [点击查看 VPC 相关信息](/product/vpc.html)
...
### 配置 CDN 加速域名
1. 登录 [万象优图控制台](http://console.tcecqpoc.fsphere.cn/ci) ，进入左侧菜单栏【 Bucket 管理】，点击需要配置域名的存储桶（如 imagetest1），进入存储桶。
...
为了防止不良网站盗用开发者的图片链接，盗取图片流量，给开发者带来经济损失，云平台万象优图推出防盗链功能，通过 HTTP 协议支持的 Referer 机制（参见 [HTTP Referer](http://en.wikipedia.org/wiki/HTTP_referer) ） 来进行相应的来源识别和管理。
登录 [万象优图控制台](http://console.tcecqpoc.fsphere.cn/ci) ，进入左侧菜单栏【 Bucket 管理】，点击需要配置域名的存储桶（如 imagetest1 ），单击【域名管理】可进行防盗链设置。
