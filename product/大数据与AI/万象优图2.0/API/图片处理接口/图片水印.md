## 简介
云平台万象优图支持实时图片水印处理功能。目前，水印图片必须指定为已存储于万象优图中的图片。
## 接口形式
