...
人脸核身服务现已开放申请，您可以[点此进行申请](/act/event/face-in.html)。
...
| compare_flag  | 必须   | bool   | video 中的照片和 card 是否做对比，  1. 参数为 True 时做对比，card 是对比的照片，会返回活体检测和对比的结果。  2.参数为 False 时只做活体检测，card 字段可不填，只返回活体检测的结果。     在设置 form-data 数据时设置为字符串"true"、"false"。 |
