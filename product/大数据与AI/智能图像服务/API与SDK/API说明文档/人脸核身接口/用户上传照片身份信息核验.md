...
### 服务简介
本接口用于对用户上传的照片进行身份校验，即根据用户的身份证号、姓名获取公安部的身份证照片，与上传的图像进行人脸相似度对比。
...
| data.similarity | float  | 用户上传的图像与从公安部获取的带网纹图像的 face 相似度，取值范围[0,100]，推荐相似度大于 75 时可判断为同一人，可根据具体场景自行调整阈值 |
| code            | int    | 错误码                                      |
| message         | string | 错误描述                                     |
