...
- **硬件存储：**对于可靠性要求极高的数据，请使用云平台云硬盘保证数据的持久存储可靠性，尽量不要选择 [本地盘](/doc/product/213/5798)。
- **数据库：**对于访问频繁、容量不稳定的数据库，可使用 [云平台云数据库](http://console.tce.futunn.com/product/cdb-overview) 。

## 备份和恢复

- **同地域备份实例：**可以使用 **自定义镜像** 以及 **CBS 快照** 的方式来备份您的实例与业务数据。详见 [创建自定义镜像](/doc/product/213/4942) 。
