...
### 如何调整安全组优先级？
登录 [控制台](http://console.tcecqpoc.fsphere.cn/cvm) - 详情页 - 安全组 - 已加入安全组 - 编辑。
...
> **注意：**
如果您发起解封申请，云平台将默认您已确认并承诺：保证 TCP 25 端口仅用来连接第三方的 SMTP 服务器，并从第三方的 SMTP 服务器向外发邮件。如发现您使用申请的 IP 直接通过 SMTP 发送邮件，云平台有权永久性封禁 TCP 25 端口，并不再提供解封服务。
