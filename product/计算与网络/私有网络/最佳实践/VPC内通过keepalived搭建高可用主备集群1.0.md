...
**具体操作：** 
1) 通过云`API:DescribeNetworkInterfaces`[点击查看API详情](/doc/api/245/4814)得到云服务器的主网卡的`networkInterfaceId`（入参填写：**私有网络ID**和**云服务器的ID**即可）。
2) 通过云`API:AssignPrivateIpAddresses`[点击查看API详情](/doc/api/245/4817)在弹性网卡上申请内网VIP的，申请VIP操作可参考以下python代码：
...
### 步骤 4.（可选）给VIP分配外网IP
先在控制台申请EIP，再通过云API绑定到1中申请的内网IP，[点击查看具体调用方式](/doc/api/229/1377)，python代码与步骤1类似。
...
step2: 将以下python代码保存成vip.py放到sdk的src同级目录,  具体参数参考: /doc/api/245/1361
"""
