...
### 控制台手动续费
通过控制台手动续费的操作步骤如下：
1. 登录 [云服务器 CVM 控制台]( http://console.qcloud.com/cvm/)。
...
###  API 续费
用户可以使用 RenewInstance 接口续费实例，具体内容可以参考 [续费实例（包年包月） API>>](http://www.qcloud.com/doc/api/229/1348)。
## 自动续费
### 控制台设置自动续费
用户可以为包年包月类型的 GPU 实例设置自动续费，避免每次面临到期时都需要进行续费操作：
1.  登录 [云平台控制台]( http://console.qcloud.com)，将鼠标移动到右上角的费用处，在菜单中选择【续费】。
...
### API 设置自动续费
用户可以使用 SetAutoRenew 接口设置自动续费实例，具体内容可以参考  [设置实例自动续费 API>>](http://www.qcloud.com/doc/api/229/1746)。
## 续费至统一时间
