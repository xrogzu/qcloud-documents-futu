...
但是生产和消费的过程中有可能存在重复生产和消费失败等现象，只通过修改队列属性无法保证队列只消费一次，这里需要业务方来实现生产消费的去重和容错。可参见[消息去重>>](http://tcecqpoc.fsphere.cn/document/product/406/8303)
...
[了解消息回溯功能 >>](http://tcecqpoc.fsphere.cn/document/product/406/8129)
