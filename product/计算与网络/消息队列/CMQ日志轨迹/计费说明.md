Queue/Topic的原始Log存放于COS对象存储中。创建Queue/Topic时，默认不启动Logging功能，若启动，需要选定某个bucket，bucket由客户自行创建。费用由云平台对象存储收取，具体的计费方式请参考 [COS 计费项总览](http://tce.fsphere.cn/doc/product/430/5871)。
