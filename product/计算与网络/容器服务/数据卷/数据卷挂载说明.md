...
- **配置文件**：将配置文件中某个 key 映射到容器中，可以用于挂载配置文件到指定容器目录，详情请参阅 [配置项概述](http://tcecqpoc.fsphere.cn/document/product/457/10173)。

**使用步骤**：
1. 在创建服务页面 **数据卷** 选项下单击【添加数据卷】。
![](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/0286498ec3ada210c6c01f9ef8ca7b52/image.png)
2. 配置数据卷。
 - **类型**：单击【∨】，在下拉框中选择数据卷的类型。
 - **名称**：数据卷的名称。
由小写字母和数字和连接符 “-” 组成，必须是小写字母开头，且在 20 个字符以内。
 - **路径**：指定容器要挂载的路径。
![](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/4495a452a8b1bbf9bb3b4e2fcb1bcf99/image.png)
3. 设置挂载路径。
在 **运行容器**下单击【显示高级设置】。
![](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/789e7fc5148ef53f44035194c5c6bd2c/image.png)
在高级设置展开页面中，单击 **挂载点** 下【∨】选择数据卷配置挂载路径。
![](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/e762feea4f8b551fa5d8dd45721e597a/image.png)
>**注意：**
>- 创建服务时，数据卷挂载可以为空。
>- 数据卷挂载的卷名称和目标路径不能重复。
