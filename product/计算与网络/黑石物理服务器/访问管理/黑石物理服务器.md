...
以下表格，罗列了在配置黑石服务器的策略时，需要用到的 action、resource、condition。相关概念请参考 [访问管理](http://tcecqpoc.fsphere.cn/document/product/598/10603"访问管理") 章节。
...
> 注意:
> 部份 API 鉴权时需要两种产品的实例 ID，例如绑定 EIP，分别需要被绑定的黑石服务器以及用于绑定的黑石弹性公网 IP 的实例 ID，这时需要把两种云产品的资源描述都写在 Resource 里。
