...
```
注意：
1. 通常情况下在本地安装的依赖库在 SCF 平台上也能很好运行，但少部分情况下安装的 binary 文件可能产生兼容性问题，如果发生了此问题请您尝试[联系我们](http://tce.fsphere.cn/document/product/583/9712)。
...
```
2) 将创建的此函数所有 Python 源文件（.py 文件）保存在此目录，有关如何创建函数，请参考 [新手入门 - 创建 DownloadImage 函数](http://tce.fsphere.cn/document/product/583/9211)部分。
...
## Windows 下创建部署程序包
建议您将已经在 Linux 环境下运行成功的依赖包和代码打包成 zip 包作为函数的执行代码，具体操作请参考 [代码实操 - 获取COS上的图像并创建缩略图](http://tce.fsphere.cn/document/product/583/9736)。
