...
- 用户代码中位于[执行方法](http://tcecqpoc.fsphere.cn/document/product/583/9210#.E6.89.A7.E8.A1.8C.E6.96.B9.E6.B3.95)外部的任何声明保持已初始化的状态，再次调用函数时可以直接重用。例如，如果您的函数代码中建立了数据库连接，容器重用时可以直接使用原始连接。您可以在代码中添加逻辑，在创建新连接之前检查是否已存在连接。
...
|最大执行时长|300 秒|
|控制台上传的代码包大小|5MB|
|最大并发容器数|20，用户可以[联系我们](http://tcecqpoc.fsphere.cn/document/product/583/9712)提高该限制|
...
默认情况下，SCF 将会对每个函数的并发量限制为 20。您可以通过[联系我们](http://tcecqpoc.fsphere.cn/document/product/583/9712)来调高此数值。
...
- [Qcloud COS SDK](http://tcecqpoc.fsphere.cn/document/product/436/6275)
- [Qcloud API SDK](http://tcecqpoc.fsphere.cn/document/developer-resource/494/7244)
