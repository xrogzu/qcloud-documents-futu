...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，请参见 [公共请求参数](/document/product/295/7279) 页面。其中，此接口的 Action 字段为 AntiFraud。
...
| 参数名称 | 类型 | 描述 |
| ----- | ----- | ----- |
| code | Int | 公共错误码</br>0：表示成功</br>其他值：表示失败</br>详见 [错误码](/document/product/295/7285) 页面中的 **公共错误码** 相关内容 |
...
- 风险码
  ...
  风险码及其说明请参见 [风险码](/document/product/668/14278) 页面。
...
一个完整的请求需要两类请求参数：公共请求参数和接口请求参数。这里只列出了接口请求参数，并未列出公共请求参数，有关公共请求参数的说明可见 [公共请求参数](/document/product/295/7279) 页面。
