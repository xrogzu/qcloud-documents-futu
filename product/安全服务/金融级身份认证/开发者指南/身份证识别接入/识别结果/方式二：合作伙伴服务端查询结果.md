...
## 1. 合作方后台生成签名
### 准备步骤
- **前置条件：请合作方确保 SIGN ticket 已经正常获取，获取方式见 [SIGN ticket 获取](/document/product/655/13815)** 。
...
```
http://idasc.webank.com/api/server/getOcrResult
```
**请求方法：**GET
**请求参数：**
...
| frontWarnCode | string         | 人像面警告码，在身份证有遮挡、缺失、信息不全时会返回告警码；当 frontCode 为 0 时才会出现告警码，告警码的含义请参考【[通用响应码列表](/document/product/655/13819) 】>【身份证 OCR 识别响应码】 |
| backWarnCode  | string         | 国徽面警告码，在身份证有遮挡、缺失、信息不全时会返回告警码；当 backCode 为 0 时才会出现告警码，告警码的含义请参考【[通用响应码列表](/document/product/655/13819) 】>【身份证 OCR 识别响应码】 |
...
> -  对于身份证 OCR 识别有部分遮挡、缺失、信息不全的情况，请参考 frontWarnCode 和 backWarnCode 告警码。（参考【[通用响应码列表](/document/product/655/13819) 】>【身份证识别响应码】）