### 1.生成签名 
1.1 **前置条件**：必须按照 [SIGN ticket获取](http://tcecqpoc.fsphere.cn/document/product/295/10120?=cn).
...
| version | 1.0.0 | 
| api ticket | 合作伙伴服务端缓存的 tikcet,注意是 SIGN 类型，具体参考 [SIGN ticket获取](http://tcecqpoc.fsphere.cn/document/product/295/10120?=cn)| 

1.3 将 webankAppId, orderNo, name, idNo, userId, version，ticket（SIGN 类型）共七个参数的值进行字典序排序。将排序后的所有参数字符串拼接成一个字符串进行 SHA1 编码。
...
| orderNo |订单号，由合作方上送，每次唯一，不能超过 32 位 | String |订单号，由合作方上送，每次唯一，不能超过 32 位 |必填 |
|name | 姓名 | String | |必填 |
|idNo | 证件号码 | String | |必填 |
...
|sourcePhotoType | 比对源照片类型 | String |1 |1：水纹正脸照2：高清正脸照 |
