>> ** 注：小程序的接入和鉴权整体流程，请参考活体识别 H5 登录鉴权 **
 ...
 ###  1.生成签名
1.  **前置条件**：必须按照必须按照 [SIGN ticket获取](http://tcecqpoc.fsphere.cn/document/product/295/10120?=cn).
...
| userId | 用户 ID ，用户的唯一标识（不要带有特殊字符）。 | 
 |version| 1.0.0 | 
| api ticket | 合作伙伴服务端缓存的 tikcet,注意是 SIGN 类型，具体见[SIGN ticket获取](http://tcecqpoc.fsphere.cn/document/product/295/10120?=cn)来获取规则。|
