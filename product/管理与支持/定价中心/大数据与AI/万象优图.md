...
![](http://imgcache.tcecqpoc.fsphere.cn/image/mc.qcloudimg.com/static/img/edab5701937c90ce21ce626cc98483d4/image.png)

【说明】
- 图片鉴黄为打分制，对于每一张图片均有一个0~100的分值
- **确定部分**指确定是黄图和确定是正常的图片，无需人工干预
- **不确定部分**指疑似色情图片，系统无法区分是否色情，分值在83~91，建议用户通过人工审核的方式确定。
- 存储在云平台的图片每日有2000张的免费鉴图额度，未用完的额度不累计至下一天，计费会扣除免费的2000张（先扣费用比较高的确定部分）。
- 计费方式为阶梯到达，月结。
