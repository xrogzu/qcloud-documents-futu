...
注册成功后，会返回设备 token，token 用于标识设备唯一性，同时也是信鸽维持与后台连接的唯一身份标识。关于如何获取 token 请参考“ [获取 token](http://tcecqpoc.fsphere.cn/document/product/548/14683) ”章节。
...
在信鸽 3.2.2 beta 版本以后解绑帐号需要调用全新的接口。详细请参考 [接口概览](http://tcecqpoc.fsphere.cn/document/product/548/13950) 。
...
### 使用 callback 版本的注册接口
XGIOperateCallback 类提供注册成功或失败的处理接口，请参考 [注册接口](http://tcecqpoc.fsphere.cn/document/product/548/13950#xgpushmanager-.E5.8A.9F.E8.83.BD.E7.B1.BB) 里面的示例。
...
>注意：
>重载的 XGPushBaseReceiver 需要配置在 AndroidManifest.xml，请参考 [消息配置](http://tcecqpoc.fsphere.cn/document/product/548/13952#.E6.B6.88.E6.81.AF.E9.85.8D.E7.BD.AE) 章节的相关内容。
