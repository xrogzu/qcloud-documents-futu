...
私有群、公开群、聊天室、互动直播聊天室和在线成员广播大群全部支持使用本REST API修改群组基础资料。

### 2.2 请求URL 
```
http://console.tim.qq.com/v4/group_open_http_svc/modify_group_base_info?usersig=xxx&identifier=admin&sdkappid=88888888&random=99999999&contenttype=json 
```
### 2.3 请求参数 
...
| 字段 | 类型 | 属性 | 说明 |
|---------|---------|---------|---------|
| GroupId | String | 必填 |需要修改基础信息的群组的ID。    |
...
| MaxMemberNum | Integer | 选填 |最大群成员数量，最大为10000。 |
| ApplyJoinOption | String | 选填 |申请加群处理方式。包含FreeAccess（自由加入），NeedPermission（需要验证），DisableApply（禁止加群）。 |
| AppDefinedData | Array | 选填 |默认情况是没有的。开通群组维度的自定义字段详情参见：[自定义字段](/doc/product/269/群组系统#6-.E8.87.AA.E5.AE.9A.E4.B9.89.E5.AD.97.E6.AE.B5)。 |
...
通过[REST API在线调试工具](http://avc.tce.fsphere.cn/im/APITester/APITester.html#v4/group_open_http_svc/modify_group_base_info)调试本接口。 
