...
1. 私有群、公开群支持使用本REST API设置成员未读消息计数；
2. 聊天室、互动直播聊天室和在线成员广播大群的客户端不支持未读消息计数，所以对这三种类型的群组成员设置未读消息计数是无效的（但是不会返回错误）。

### 2.2 请求URL 
```
http://console.tim.qq.com/v4/group_open_http_svc/set_unread_msg_num?usersig=xxx&identifier=admin&sdkappid=88888888&random=99999999&contenttype=json
```
### 2.3 请求参数 
...
通过[REST API在线调试工具](http://avc.tce.fsphere.cn/im/APITester/APITester.html#v4/group_open_http_svc/set_unread_msg_num)调试本接口。 
