...
私有群、公开群、聊天室和互动直播聊天室全部支持使用本REST API对用户批量禁言和取消禁言。而在线成员广播大群只有APP管理员可以发送消息，所以无需支持设置和取消禁言。

### 2.2 请求URL 
```
http://console.tim.qq.com/v4/group_open_http_svc/forbid_send_msg?usersig=xxx&identifier=admin&sdkappid=88888888&random=99999999&contenttype=json 
``` 
### 2.3 请求参数 
...
| Members_Account | Array | 必填 |需要禁言的用户账号，最多支持500个账号。   |
| ShutUpTime | Integer | 必填 |需禁言时间，单位为秒，为0时表示取消禁言。   |
...
通过[REST API在线调试工具](http://avc.tce.fsphere.cn/im/APITester/APITester.html#v4/group_open_http_svc/forbid_send_msg)调试本接口。 
