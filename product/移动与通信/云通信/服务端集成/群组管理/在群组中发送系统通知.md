...
私有群、公开群、聊天室、互动直播聊天室和在线成员广播大群全部支持使用本REST API发送系统通知。
特别的，私有群、公开群、聊天室支持向群组中的一部分指定成员发送系统通知，而互动直播聊天室和在线成员广播大群只支持向群组中所有成员发送系统通知。

### 2.2 请求URL 
```
http://console.tim.qq.com/v4/group_open_http_svc/send_group_system_notification?usersig=xxx&identifier=admin&sdkappid=88888888&random=99999999&contenttype=json 
```
### 2.3 请求参数 
...
通过[REST API在线调试工具](http://avc.tce.fsphere.cn/im/APITester/APITester.html#v4/group_open_http_svc/send_group_system_notification)调试本接口。 
