...
1. 该API接口v4/openim/importmsg作用是导入历史单聊消息到云平台；
2. 平滑过渡期间，将原有即时通信实时单聊消息导入到云平台；
3. 该接口不会触发回调。
...
### 2.1 请求URL
```
http://console.tim.qq.com/v4/openim/importmsg?usersig=xxx&identifier=admin&sdkappid=88888888&random=99999999&contenttype=json
```
### 2.2 请求参数
...
通过[REST API在线调试工具](http://avc.qcloud.com/im/APITester/APITester.html#v4/openim/importmsg)调试本接口。 
