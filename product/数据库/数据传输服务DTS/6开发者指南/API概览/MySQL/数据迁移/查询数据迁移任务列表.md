...
## 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见<a href='/doc/api/372/4153' title='公共请求参数'>公共请求参数</a>页面。其中，此接口的 Action 字段为 GetCdbDataMigrationTaskList。

| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| jobId | 否 | Int | 数据迁移任务 ID，通过该 ID 可获得其执行详情 |
| jobName | 否 | String | 数据迁移任务名称 |
| status | 否 | Array | 数据迁移任务状态集合，可取值包括：<br>0-未配置<br>1-已配置<br>2-校验中<br>3-校验失败<br>4-校验成功<br>5-准备迁移<br>6-迁移中<br>7-待切换<br>8-迁移失败<br>9-迁移成功 |
| migrateType | 否 | Int |数据迁移类型，值包括：<br>1-结构迁移<br>2-全量迁移<br>3-全量+增量迁移 |
| dstUInstanceId | 否 | String | 目标实例 ID，格式如：cdb-c1nl9rpv，与云数据库控制台页面中显示的实例 ID 相同，可使用[查询实例列表](/doc/api/253/1266) 接口获取，其值为输出参数中字段 uInstanceId 的值。 |
| type | 否 | Array | 任务类型，值包括：<br>1-源实例为公网自建MySQL，<br>2-源实例为基础网络自建 MySQL，<br>3-源实例为 VPC 网络自建MySQL，<br>4-源实例通过专线接入，<br>5-源实例通过云 VPN 接入，<br>6-源实例通过自建VPN接入，<br>7-源实例为 CDB 实例 |
| runMode | 否 | Int | 任务运行模式，值包括：1-立即执行，2-定时执行|
| order | 否 | String | 排序字段，可以取值为 jobId、status、jobName、migrateType、type、runMode、submitTime |
...
| data | Object | 返回的数据 |
| data.totalCount | Int | 满足查询条件的数据迁移任务条数 |
| data.jobList | Array | 数据迁移任务分页列表 |

其中， jobList 中每一个元素为Object，其构成如下：

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| code | Int | 数据迁移任务执行状态码，0 表示成功，其他表示失败 |
| message | String | 数据迁移任务执行状态信息 |
| jobId | Int | 数据迁移任务 ID |
| type | Int | 数据迁移任务类型，值包括：<br>0-未配置，<br>1-源实例为公网自建 MySQL，<br>2-源实例为基础网络自建 MySQL，<br>3-源实例为VPC网络自建MySQL，<br>4-源实例通过专线接入，<br>5-源实例通过云VPN接入，<br>6-源实例通过自建VPN接入，<br>7-源实例为CDB实例|
| status | Int | 数据迁移任务执行状态，值包括：<br>0-未配置<br>1-已配置<br>2-校验中<br>3-校验失败<br>4-校验成功<br>5-准备迁移<br>6-迁移中<br>7-待切换<br>8-迁移失败<br>9-迁移成功 |
| progress | Int | 数据迁移任务执行进度 |
| startTime | String | 数据迁移任务开始时间，格式为 yyyy-mm-dd hh:mm:ss |
| EndTime | String | 数据迁移任务结束时间，格式为 yyyy-mm-dd hh:mm:ss |
...
| stepInfo | Object | 数据迁移任务执行过程的步骤信息，具体参见示例 |
...
输入
<pre>
http://cdb.api.qcloud.com/v2/index.php?Action=GetCdbDataMigrationTaskList
