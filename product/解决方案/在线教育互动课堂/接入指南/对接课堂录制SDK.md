...
开始录制课堂视频前，需要下载历史的白板操作数据和音视频录制数据。可以由业务服务器使用云通信的 API 接口 [拉取群漫游消息](http://tcecqpoc.fsphere.cn/document/product/269/2738) 下载指定群组的漫游消息后，下发给课堂录制客户端。
...
## 二. 记录课堂录制结果
课堂录制客户端完成录制后，实时音视频服务器会将课堂视频的 URL 信息回调给业务服务器，业务服务器可以获取到课堂视频的 URL 信息。由于实时音视频后台会每 30 分钟生成一次录制文件，所以业务后台要等待所有录制文件的 URL 全部回调后才可以结束记录。
