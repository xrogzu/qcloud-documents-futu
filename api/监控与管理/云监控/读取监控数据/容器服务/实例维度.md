...
容器服务为用户解决容器集群的搭建以及运维管理工作，无缝衔接了云平台的计算、网络、存储、监控、安全能力，帮助用户升级开发模式、变更应用交付、重构数据管理方式。助力企业快速上云、提速应用部署、简化集群管理，将精力花在刀刃上。具体介绍请参考<a href="/document/product/457/6759">容器服务产品概述</a>页面。
...
| 英文          | 中文含义                                     |
| ----------- | ---------------------------------------- |
| clusterId   | 请填写[查询集群列表](/document/api/457/9448)接口中返回的clusterId(集群的Id)字段 |
| serviceName | 请填写[查询服务列表](/document/api/457/9440)接口中返回的serviceName(服务名)字段 |
| namespace   | 请填写[查询服务列表](/document/api/457/9440)接口中返回的namespace(命名空间)字段 |
| podName     | 请填写[查询服务实例列表](/document/api/457/9433)接口中返回的name(实例名称)字段 |
