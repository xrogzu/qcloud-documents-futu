...
对象存储服务 COS（Cloud Object Storage）是云平台提供的一种基于 Web 的安全稳定、海量、便捷、低延迟、低成本的云端存储服务，有关产品信息请参考 [对象存储产品页](http://console.tcecqpoc.fsphere.cn/cos) 介绍。
...
dimensions.0.value为根账户的开发商 APPID

dimensions.1.name=bucket

dimensions.1.value为存储桶名称
...
| 参数名称               | 维度名称   | 维度解释      | 格式                          |
| ------------------ | ------ | --------- | --------------------------- |
| dimensions.0.name  | appid  | 开发商 APPID | String类型维度名称：appid          |
| dimensions.0.value | appid  | 开发商 APPID | 根账户开发商 APPID，如 125232125    |
| dimensions.1.name  | bucket | 存储桶名称     | String类型维度名称：bucket         |
| dimensions.1.value | bucket | 存储桶名称     | 存储桶名，如 testbucket-125232125 |
