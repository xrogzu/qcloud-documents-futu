...
dimensions.0.value为云服务器ID，调用[DescribeInstances](http://tcecqpoc.fsphere.cn/doc/api/229/%E6%9F%A5%E7%9C%8B%E5%AE%9E%E4%BE%8B%E5%88%97%E8%A1%A8)接口获取的unInstanceId字段。
...
### 6.13 容器服务
云平台容器服务为用户解决容器集群的搭建以及运维管理工作，无缝衔接了云平台的计算、网络、存储、监控、安全能力，帮助用户升级开发模式、变更应用交付、重构数据管理方式。助力企业快速上云、提速应用部署、简化集群管理，将精力花在刀刃上。具体介绍请参考<a href="http://tcecqpoc.fsphere.cn/document/product/457/6759">容器服务产品概述</a>页面。
...
dimensions.0.value为集群ID，请填写[查询集群列表](http://tcecqpoc.fsphere.cn/document/api/457/9448)接口中返回的clusterId(集群的Id)字段
...
dimensions.1.value为服务名称，请填写[查询服务列表](http://tcecqpoc.fsphere.cn/document/api/457/9440)接口中返回的serviceName(服务名)字段

dimensions.2.name=namespace
dimensions.2.value为命名空间名称，请填写[查询服务列表](http://tcecqpoc.fsphere.cn/document/api/457/9440)接口中返回的namespace(命名空间)字段
...
dimensions.3.value为实例名称，请填写[查询服务实例列表](http://tcecqpoc.fsphere.cn/document/api/457/9433)接口中返回的name(实例名称)字段
...
dimensions.4.value为容器ID，请填写[查询服务实例列表](http://tcecqpoc.fsphere.cn/document/api/457/9433)接口中返回的containerId(容器id)字段。<font style="color:red"><strong>注意：容器Id只用取开头12位即可</strong></font>
