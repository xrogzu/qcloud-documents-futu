...
## 1. 申请安全凭证
在第一次使用云API之前，用户需要在云平台CVM控制台上申请安全凭证。
安全凭证包括 SecretId 和 SecretKey，其中：</br>
>SecretId:用于标识 API 调用者身份;</br>
>SecretKey:用于加密签名字符串和服务器端验证签名字符串的密钥。</br>
<font color='red'>注意：API 密钥是构建云平台 API 请求的重要凭证，使用云平台 API 可以操作您名下的所有云平台资源，为了您的财产和服务安全，请妥善保存和定期更换密钥，当您更换密钥后，请及时删除旧密钥。</font>
...
3) 在[云API访问密钥管理](http://console.tce.fsphere.cn/capi)页面，点击【新建】即可以创建一对SecretId/SecretKey。
...
以[查询实例列表](/document/product/236/1266)(DescribeCdbInstances)请求为例，当用户调用这一接口时，其请求参数可能如下:
...
* 请求主机:[查询实例列表](/document/product/236/1266)(DescribeCdbInstances)的请求域名为：cdb.api.qcloud.com。实际的请求域名根据接口所属模块的不同而不同, 详见各接口说明。
...
### 2.4. 生成签名串
此步骤生成签名串。
<font color='red'>注意：这里要根据您指定的签名算法（即SignatureMethod参数）生成签名串。当指定SignatureMethod为HmacSHA256时，需要使用HmacSHA256计算签名，其他情况请使用HmacSHA1计算签名。</font>
...
<font color='red'>注意：如果用户的请求方法是GET，则对所有请求参数的参数值均需要做URL编码；此外，部分语言库会自动对URL进行编码，重复编码会导致签名校验失败。</font>
...
| 4102 | 未被开发商授权访问本接口中所操作的资源 | 请问的资源参数中，存在未被开发商授权授权访问的资源，请在message字段中查看无权查看的资源ID。</br>请联系开发商授权，详情请查阅[授权策略](/doc/product/378/4513)。|
