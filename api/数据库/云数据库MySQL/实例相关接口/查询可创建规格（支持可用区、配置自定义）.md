...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见<a href='/document/product/236/6921' title='公共请求参数'>公共请求参数</a>页面。其中，此接口的Action字段为DescribeCdbProductListNew。
...
| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| timeSpan | Array | 当前用户、地域下可购买的时间，单位：月 |
