...
可以在云监控中[MySQL监控数据接口](/document/api/248/11006)通过云API来获取CDB实例的监控指标。


## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见 [公共参数](/document/api/236/6921) 页面。其中，此接口的Action字段为GetCdbDeviceMonitorInfo。
...
输入
<pre>
http://cdb.api.qcloud.com/v2/index.php?Action=GetCdbDeviceMonitorInfo
