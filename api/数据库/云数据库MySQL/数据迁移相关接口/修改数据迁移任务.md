...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见<a href='/doc/api/372/4153' title='公共请求参数'>公共请求参数</a>页面。其中，此接口的Action字段为ModifyCdbDataMigrationTask，此接口的Region字段为目标实例的地域值。

| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| jobId | 是 | Int | 待修改的数据迁移任务ID，请使用查询数据迁移任务列表接口查询任务ID |
...
| dstInfo | 是 | Object | 目的实例信息，仅包含一个属性cdbUInstanceId，格式如：cdb-c1nl9rpv，与云数据库控制台页面中显示的实例ID相同，可使用[查询实例列表](/doc/api/253/1266) 接口获取，其值为输出参数中字段 uInstanceId 的值。|
