...
本实例展示如何创建一个实例：首先查询实例购买规格；然后通过创建接口创建实例；最后，通过订单查询接口，查询实例创建进度。
...
## 2. 创建副本集实例
接下来，我们按照上面的规格，调用[创建实例](/document/product/240/7089)接口, 输入参数如下： 
...
| projectId | 否 | Int | 项目id，取值以用户账户>用户账户相关接口查询>项目列表 返回的projectId为准|0 |
...
| details.dealName | String | 长订单号ID |
