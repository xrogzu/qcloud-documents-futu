## 1. 接口描述
本接口(DescribeMonitoringData)用于查询监控数据。
接口请求域名：<font style='color:red'>postgres.api.qcloud.com </font>

## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见<a href='/doc/api/238/7328' title='公共请求参数'>公共请求参数</a>页面。其中，此接口的Action字段为DescribeMonitoringData。

| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| Version | 是 | String | 表示API版本号，主要用于标识请求的不同API版本。 本接口第一版本可传：2017-03-12。|
| ResourceId | 是 | String | 实例ID，格式如：postgres-9yyzeg2z |
...
输入
<pre>
http://postgres.api.qcloud.com/v2/index.php?Action=DescribeMonitoringData
&Version=2017-03-12
&<<a href="http://www.qcloud.com/document/api/238/7328">公共请求参数</a>>
&ResourceId=postgres-9yyzeg2z
</pre>
输出
```
{
