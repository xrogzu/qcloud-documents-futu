...
| taskID  | String | 任务ID，云平台生成全局唯一的任务 ID，有效期一个月，一个月之后任务失效。可以调用[获取创建多设备任务状态](http://tce.fsphere.cn/document/product/634/14069)接口获取该任务的执行状态，当状态为成功时，可以调用[获取创建多设备任务结果](http://tce.fsphere.cn/document/product/634/14070)接口获取该任务的结果    |
