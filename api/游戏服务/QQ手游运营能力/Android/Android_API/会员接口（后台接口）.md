...
其中 URL 为 `http://mq.vip.qq.com/m/connect/open`，参数以 GET 参数形式传入，例如：`http://mq.vip.qq.com/m/connect/open?code=svip&month=1&aid=mvip.p.hz.demo`
...
```
WebViewHelper.getInstance().setTencent(mTencent).openUrl(v.getContext(), "http://mq.vip.qq.com/m/connect/open?code=svip&month=1&aid=mvip.p.hz.demo", ActivityInfo.SCREEN_ORIENTATION_SENSOR);
```
