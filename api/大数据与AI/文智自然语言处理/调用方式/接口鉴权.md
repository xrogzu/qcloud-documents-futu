...
以[查看实例列表](http://tce.fsphere.cn/document/api/213/9388 )(DescribeInstances)请求为例，当用户调用这一接口时，其请求参数可能如下:
...
2) 请求主机:[查看实例列表](http://tce.fsphere.cn/document/api/213/9388 )(DescribeInstances)的请求域名为：cvm.api.qcloud.com。实际的请求域名根据接口所属模块的不同而不同, 详见各接口说明。
