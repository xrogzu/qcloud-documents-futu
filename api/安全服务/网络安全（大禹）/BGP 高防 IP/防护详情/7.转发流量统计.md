## 接口描述
用于获取 BGP 高防 IP 向云平台外主机转发流量的统计图表的接口，只有云平台外属性的 IP 该接口才有效。
协议：`HTTPS`
域名：`bgpip.api.qcloud.com`
接口名：`BGPIPTransGetStatistics`

## 请求参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，详情请参见 [公共请求参数说明](http://tcecqpoc.fsphere.cn/document/api/213/6976)。其中，此接口的 Action 字段为 `BGPIPTransGetStatistics`。
