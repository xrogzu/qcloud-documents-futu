...
## 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见 [公共请求参数](http://tcecqpoc.fsphere.cn/document/product/570/13932 "点击查看公共请求参数")页面。其中，此接口的 Action 字段为 UpdateDsaHostInfo。

| 参数名称   | 必选 | 类型     | 描述                                       |
| ------ | ---- | ------ | ---------------------------------------- |
| hostId | 是    | Int    | 域名接入 DSA 后的标识 ID，可通过 [查询域名列表信息](http://tcecqpoc.fsphere.cn/document/product/570/13940) 接口获取  |
