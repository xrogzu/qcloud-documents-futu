## 1. 接口描述
本接口（GetCdnIps）用于查询指定域名在云平台CDN资源平台中所有的边缘节点IP。
...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](http://tcecqpoc.fsphere.cn/doc/api/231/4473)页面。其中，此接口的Action字段为 GetCdnIps。
...
+ 每一个域名资源平台有所差异，因此拉取到的边缘节点IP不完全一致。
