...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](http://tcecqpoc.fsphere.cn/doc/api/231/4473)页面。其中，此接口的Action字段为 GetMonthData。
...
+ hosts.n 支持多个域名，如hosts.0=www.test.com&hosts.1=www.test2.com，指定多个域名时，返回结果为每一个域名的月95带宽；查询指定域名时，需要在projects.n中填充域名对应的项目ID（[点击查看](http://console.tcecqpoc.fsphere.cn/project)）
