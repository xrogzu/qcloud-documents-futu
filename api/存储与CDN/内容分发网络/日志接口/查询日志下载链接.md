...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](http://tce.fsphere.cn/doc/api/231/4473)页面。其中，此接口的Action字段为GenerateLogList。
...
+ 未填充startDate和endDate时，默认查询区间为当前时间前推30天，每小时提供一个日志下载链接，更多日志相关说明可查看[日志下载](http://tce.fsphere.cn/doc/product/228/6316) ；
