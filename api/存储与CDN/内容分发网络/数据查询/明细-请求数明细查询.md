<font style="color:Brown">接口 GetCdnHostsDetailStatistics 通过入参 statType 不同可指定查询带宽、流量、请求数、命中请求数、命中流量、状态码等明细数据，此文档对请求数明细查询方式进行了详细说明</font>
...
+ 可一次提交多个域名（最多 10 个），查询每一个域名的实时请求数明细数据，及每一个域名累加后的实时请求数明细数据
+ 可一次提交一个或多个项目ID，获取一个项目，或多个项目累加后的实时请求数明细数据
