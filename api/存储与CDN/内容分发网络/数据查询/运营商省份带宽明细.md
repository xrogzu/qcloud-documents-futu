...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](/doc/api/231/4473)页面。其中，此接口的Action字段为GetCdnProvIspDetailStat。

| 参数名称    | 是否必选 | 类型     | 描述                                       |
| ------- | ---- | ------ | ---------------------------------------- |
| date    | 是    | String | 查询日期，格式必须为：2016-09-28                    |
| hosts.n | 是    | String | 域名，支持多个，如hosts.0=www.test.com&hosts.1=www.test2.com，最多可查询5个 |
| provs.n | 否    | String | 指定省份，省份通过编码区分，点击 [CDN日志省份编码映射表](/document/product/228/6316) 查看 |
| isps.n  | 否    | String | 指定运营商，运营商通过编码区分，点击[CDN日志运营商编码映射表](/document/product/228/6316) 查看 |
