...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](http://tcecqpoc.fsphere.cn/doc/api/231/4473)页面。其中，此接口的 Action 字段为 AddCdnHost。

| 参数名称         | 类型     | 是否必选 | 描述                                       |
| ------------ | ------ | ---- | ---------------------------------------- |
| host         | String | 是    | 需要接入的 CDN 域名                             |
| projectId    | String | 是    | 项目 ID，指域名要添加入的项目的对应 ID，[查看已有项目及ID](http://console.tce.fsphere.cn/project) |
