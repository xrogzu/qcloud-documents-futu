...
+ 选择托管证书，使用接口 [查询托管证书列表](http://tce.fsphere.cn/document/product/228/12543) 获取对应的证书ID
...
## 入参说明
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](http://tce.fsphere.cn/doc/api/231/4473)页面。其中，此接口的 Action 字段为 SetHttpsInfo。
...
| certId      | 否    | String | 证书ID，可通过接口 [查询托管证书列表](http://tce.fsphere.cn/document/product/228/12543) 获取 |
