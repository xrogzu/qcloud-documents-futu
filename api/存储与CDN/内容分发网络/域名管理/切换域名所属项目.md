...
+ 项目为云平台各产品共享，您可以前往[项目管理](http://console.tce.fsphere.cn/project)查看项目对应的ID
+ 调用频次限制为 100次/分钟

[查看调用示例](http://tcecqpoc.fsphere.cn/document/product/228/1734)

## 入参说明
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](http://tcecqpoc.fsphere.cn/doc/api/231/4473)页面。其中，此接口的 Action 字段为 UpdateCdnProject。
