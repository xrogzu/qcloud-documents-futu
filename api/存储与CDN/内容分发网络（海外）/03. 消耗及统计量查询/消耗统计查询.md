...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](http://tcecqpoc.fsphere.cn/doc/api/231/4473)页面。其中，此接口的Action字段为 GetCdnOverseaStat。
...
| bandwidth      | Int    | 当日峰值带宽，单位为bps                            |
| bandwidth95    | Int    | 当日95带宽值，单位为bps                           |
| flux           | Int    | 单日流量总累计量，单位为Byte                         |
...
+ flux_cache为Float类型的数组，从指定查询日期0点开始，每5分钟一个数据；
+ 00:00:00 的统计量代表了 00:00:00 - 00:04:59 期间的统计量。
