...
使用 API 对大于 20 MB 的文件进行逐片上传之前，需要先使用「初始化分片上传」API，从云平台获取到分片上传所需的参数（session 和 offset）。
...
1. 发送「初始化分片上传」请求。
2. 云平台会根据文件 SHA-1 校验值判断「秒传」状态。若成功匹配则返回该文件的 URL，秒传完成；若失败，则需要进行分片上传流程，云平台会将分片上传所需的参数（session 和 offset）返回给请求者。
