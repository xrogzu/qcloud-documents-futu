...
请确保在使用这些接口前，已充分了解了[CMQ产品说明](http://tce.fsphere.cn/doc/product/406)。
...
| 术语 | 全称  | 中文 | 说明 |
|---------|---------|---------|---------|
| CMQ | Cloud Message Queue | 云消息服务 | 云平台的消息服务包括了队列模型，主题模型，高性能消息服务等等。|
...
用户可以通过[CMQ SDK](http://tce.fsphere.cn/product/cmq#sdk) （推荐，目前提供多个语言版本），或直接调用云API（较为麻烦，建议SDK语言之外的用户使用）使用CMQ的服务：
