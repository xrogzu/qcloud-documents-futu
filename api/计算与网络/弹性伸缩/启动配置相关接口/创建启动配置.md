...
| cpu | 是 | Int | CPU核数，各个地域可选的CPU核数不一样，详情见文档[CVM实例配置](/doc/product/213/2177)。|
| mem | 是 | Int | 内存大小，单位为GB，各个地域可选的内存大小不一样，详情见文档[CVM实例配置](/doc/product/213/2177)。|
...
目前启动配置支持标准型、高IO型、内存型三种云主机（以每个地区实际开放机型为准），启动配置支持云主机的CPU和内存大小详情见[CVM实例配置](/doc/product/213/2177)。
