...
## 请求参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见 [公共请求参数](/document/api/214/4183) 页面。其中，此接口的 Action 字段为  DescribeForwardLBListeners。
 
|参数名称|必选|类型|描述|
|-----|----|----|------------|
|loadBalancerId|是|String|负载均衡实例 ID，可通过 <a href="/document/api/214/1261" title="DescribeLoadBalancers" target="_blank">DescribeLoadBalancers</a> 接口同时入参 forward 字段为 1 或者 -1 来查询。|
...
|httpCode|Int|健康检查状态码。具体可参考 [创建监听器](/document/api/214/1255) 中对该字段的解释。|
...
请求
```
http://lb.api.qcloud.com/v2/index.php?Action=DescribeForwardLBListeners
&<公共请求参数>
