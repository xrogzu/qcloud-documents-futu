...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见 [公共请求参数](http://tcecqpoc.fsphere.cn/document/api/214/4183) 页面。其中，此接口的 Action 字段为 ModifyForwardLBFourthListener。
...
|sessionExpire|否|Int|会话保持时间，0 表示关闭。可选值：30~3600，单位：秒。|
...
请求
```
http://lb.api.qcloud.com/v2/index.php?Action=ModifyForwardLBFourthListener
&<公共请求参数>
