## 最终的请求 URL 的组成
最终的请求 URL 由以下几部分组成：
- **请求域名：**[查询私有网络列表](http://tce.fsphere.cn/document/product/215/1372)(DescribeVpcEx)的请求域名为`vpc.api.qcloud.com`。实际的请求域名根据接口所属模块的不同而不同, 详见各接口说明。
...
- 正式使用时，建议使用 [官网提供的 SDK](http://tce.fsphere.cn/document/developer-resource)，如`pip install qcloudapi-sdk-python` 。 使用  SDK 时，可以方便的填写参数，而后实现云 API 调用，优于本示例程序。
