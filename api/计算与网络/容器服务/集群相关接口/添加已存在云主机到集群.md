...
* 当前云主机状态为正常或关机，才能调用此接口。具体可以参考 [查看实例列表](/doc/api/236/1266) 页面中的实例状态列表。
...
|instanceIds| 请填写 [查看实例列表](/doc/api/236/1266) 接口中返回的 unInstanceId ( 实例 ID )字段| String| 是|
...
| keyId| [密钥](/doc/product/213/503) ID。关联密钥后可使用密钥登录实例，keyId 可通过接口 [查询密钥](/doc/api/213/1946) 获取，密钥与密码不能同时指定，同时 Windows 操作系统不支持指定密钥|String|否|  
...
输入
```
  http://domain/v2/index.php?Action=AddClusterInstancesFromExistedCvm
