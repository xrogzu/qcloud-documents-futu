...
* 如需要更改带宽，请在节点创建成功后，使用接口 [UpdateInstanceBandwidthHour](http://tcecqpoc.fsphere.cn/doc/api/229/1345)  更改，**公网带宽不指定默认为 0**。
* 支持的节点类型 **(每个可用区购买的机型不相同)**，详情参见 [CVM 实例配置](http://tcecqpoc.fsphere.cn/doc/product/213/CVM%E5%AE%9E%E4%BE%8B%E9%85%8D%E7%BD%AE)。
...
| zoneId| 可用区。请填写 [查询可用区](http://tcecqpoc.fsphere.cn/document/api/213/9455) 接口中返回的 Zone 字段|String|是| 
