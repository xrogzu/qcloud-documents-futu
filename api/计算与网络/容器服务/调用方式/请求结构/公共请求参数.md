...
</td><td> 区域参数，用来标识希望操作哪个区域的实例。各区域的参数值为: <br>北京:bj，广州:gz，上海:sh。、<br><B>注意：正常情况下此参数是必须的，如无需传入，则会在相应接口中进行说明。</B>
</td><td> 否
</td></tr>
<tr>
<td> Timestamp
...
</td><td> 用来说明Signature签名使用的签名算法，字段值可为"HmacSHA256"和"HmacSHA1"。目前支持HmacSHA256和HmacSHA1两种签名算法，用户不传该字段默认使用HmacSHA1签名算法。若用户填写的字段值错误，则也默认使用HmacSHA1签名算法。
</td><td> 否
</td></tr>
<tr>
<td> SecretId
