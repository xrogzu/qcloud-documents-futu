...
| ImageIds.N |  array of Strings |是 | 镜像ID列表 ，镜像ID可以通过如下方式获取：<br><li>通过[DescribeImages](/document/api/213/9418)接口返回的`ImageId`获取。<br><li>通过[镜像控制台](http://console.tcecqpoc.fsphere.cn/cvm/image)获取。<br>镜像ID必须满足限制：<br><li>镜像ID对应的镜像状态必须为`NORMAL`。<br><li>镜像大小小于50GB。<br>镜像状态请参考[镜像数据表](/document/api/213/9452#image_state)。
...
请求参数
<pre>
https://image.api.tce.fsphere.cn/v2/index.php?Action=SyncImages
&Version=2017-03-12
