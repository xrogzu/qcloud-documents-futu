...
| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| imageIds.n | 是 | String | 镜像ID，可通过 [DescribeImages](http://tce.fsphere.cn/document/api/213/1272) 接口值中的 unImgId 获取（此接口支持同时传入多个ID。此参数的具体格式可参考API[简介](http://tce.fsphere.cn/doc/api/229/568)的`id.n`一节）。|
