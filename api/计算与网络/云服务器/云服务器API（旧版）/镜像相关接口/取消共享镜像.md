...
* 要完全取消分享，请先[查询共享人](http://tce.fsphere.cn/doc/api/229/2391)获取该镜像共享给的全部账户，并在调用时全部指定。
...
| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|---------|
| imageId | 是 | String | 镜像ID，可通过 [DescribeImages](http://tce.fsphere.cn/document/api/213/1272) 接口返回字段中的 unImgId 获取。|
| uinList.n | 是 | String | 被取消共享的账号（此接口支持同时传入多个ID。此参数的具体格式可参考API[简介](http://tce.fsphere.cn/doc/api/229/568)的`id.n`一节）。|
