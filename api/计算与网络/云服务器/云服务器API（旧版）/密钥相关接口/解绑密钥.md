...
* 如果原来没有设置密码，解绑后将无法使用SSH登录。可以调用 [ResetInstancePassword](http://tcecqpoc.fsphere.cn/doc/api/229/1245) 来增加一个登陆密码。
...
|---------|---------|---------|---------|
| keyId  | 是 | String | 密钥ID。 |
| instanceIds.n  | 是 | String | 实例ID（此接口支持同时传入多个ID解绑多个实例。此参数的具体格式可参考API[简介](http://tcecqpoc.fsphere.cn/doc/api/229/568)的`id.n`一节）。 |
