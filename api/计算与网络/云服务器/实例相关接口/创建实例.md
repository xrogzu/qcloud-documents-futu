...
|InstanceType|String|否|实例机型。不同实例机型指定了不同的资源规格，具体取值可通过调用接口 [DescribeInstanceTypeConfigs](/document/api/213/9391) 来获得最新的规格表或参见[实例类型](/document/product/213/11518)描述。若不指定该参数，则默认机型为S1.SMALL1。|
|ImageId|String|是|指定有效的[镜像](/document/product/213/4940)ID，格式形如`img-xxx`。镜像类型分为四种：<br/><li>公共镜像</li><li>自定义镜像</li><li>共享镜像</li><br/>可通过以下方式获取可用的镜像ID：<br/><li>`公共镜像`、`自定义镜像`、`共享镜像`的镜像ID可通过登录[控制台](http://console.tcecqpoc.fsphere.cn/cvm/image?rid=1&imageType=PUBLIC_IMAGE)查询。</li><li>通过调用接口 [DescribeImages](/document/api/213/9418) ，取返回信息中的`ImageId`字段。</li>|
...
#### 请求参数
<pre>
  https://cvm.api.tce.fsphere.cn/v2/index.php?Action=RunInstances
  &Version=2017-03-12
