...
| ForceReboot| Boolean| 否 |是否在正常重启失败后选择强制重启实例。取值范围：<br><li>TRUE：表示在正常重启失败后进行强制重启<br><li>FALSE：表示在正常重启失败后不进行强制重启<br><br>默认取值：FALSE。|
...
输入
<pre>
https://cvm.api.tce.fsphere.cn/v2/index.php?Action=RebootInstances
&Version=2017-03-12
