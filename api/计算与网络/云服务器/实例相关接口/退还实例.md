...
* 不再使用的实例，可通过本接口主动退还。
* 按量计费的实例通过本接口可直接退还；包年包月实例如符合[退还规则](http://tcecqpoc.fsphere.cn/document/product/213/9711)，也可通过本接口主动退还。
...
|InvalidInstanceNotSupportedPrepaidInstance|该实例不满足包月[退还规则](http://tcecqpoc.fsphere.cn/document/product/213/9711)。|
...
输入
<pre>
http://cvm.api.qcloud.com/v2/index.php?Action=TerminateInstances
&Version=2017-03-12
