...
| 名称      |    类型 |  是否必选 | 描述|
|---------|---------|---------|---------|
|VpcId|String|是|私有网络ID，形如`vpc-xxx`。有效的VpcId可通过登录[控制台](https://console.tce.fsphere.cn/vpc/vpc?rid=1)查询；也可以调用接口 [DescribeVpcEx](/document/api/215/1372) ，从接口返回中的`unVpcId`字段获取。|
|SubnetId|String|是|私有网络子网ID，形如`subnet-xxx`。有效的私有网络子网ID可通过登录[控制台](https://console.tce.fsphere.cn/vpc/subnet?rid=1)查询；也可以调用接口  [DescribeSubnetEx](/document/api/215/1371) ，从接口返回中的`unSubnetId`字段获取。|
