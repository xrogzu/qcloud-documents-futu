...
本接口(CreateSnapshot)用于对指定云硬盘创建快照，此接口会返回新创建快照的ID，用户可使用此ID调用[DescribeSnapshots(查询快照列表)](http://tcecqpoc.fsphere.cn/doc/api/364/2530)接口来查询快照创建进度(percent)。快照说明详见[快照作用](http://tcecqpoc.fsphere.cn/doc/product/213/502)。

接口请求域名：<font style="color:red">snapshot.api.qcloud.com</font>

使用限制：<br>
1. 只有具有快照能力的云硬盘才能创建快照。云硬盘是否具有快照能力可由[DescribeCbsStorages(查询云硬盘信息)](http://tcecqpoc.fsphere.cn/doc/api/364/2519)接口查询，见输出参数中`snapshotAbility`字段。<br>
2. 可创建快照数量限制见[产品使用限制](http://tcecqpoc.fsphere.cn/doc/product/362/5145)。
...
| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| storageId | 是 | String | 需要创建快照的云硬盘ID，可通过[DescribeCbsStorages(查询云硬盘信息)](http://tcecqpoc.fsphere.cn/doc/api/364/2519)接口查询 |
