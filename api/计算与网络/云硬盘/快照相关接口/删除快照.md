...
使用限制：<br>
1. 快照必须处于`normal`状态，快照状态可以通过[DescribeSnapshots(查询快照列表)](http://tce.fsphere.cn/doc/api/364/2530)接口查询，见输出参数中`status`字段解释。 
...
| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| snapshotIds.n  | 是 | Array[String] | 快照ID，可通过[DescribeSnapshots(查询快照列表)](http://tce.fsphere.cn/doc/api/364/2530)查询|
