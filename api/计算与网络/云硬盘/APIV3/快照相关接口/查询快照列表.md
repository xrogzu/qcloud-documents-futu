...
| project-id | Integer | 否 |（过滤条件）按云硬盘所属项目ID过滤。可通过调用 [DescribeProject](/document/api/378/4400) 查询已创建的项目列表或登录[控制台](http://console.tce.fsphere.cn/project)进行查看；也可以调用 [AddProject](/document/api/378/4398) 创建新的项目。|
