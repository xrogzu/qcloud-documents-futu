...
ModifyBmLocationBackendPort 提供了修改黑石负载均衡七层转发路径后端实例端口功能。
...
```
GET http://bmlb.api.qcloud.com/v2/index.php?Action=ModifyBmLocationBackendPort
	&<公共请求参数>
...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数页面](/document/product/386/6718)。其中，此接口的Action字段为 ModifyBmLocationBackendPort。
