...
```
GET http://bmlb.api.qcloud.com/v2/index.php?Action=ModifyBmForwardLocation
	&<公共请求参数>
...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数页面](/document/product/386/6718)。其中，此接口的Action字段为 ModifyBmForwardLocation。
...
| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
|rules.n.domainId|是|String|转发域名实例ID，可通过接口[DescribeBmForwardRules](/document/product/386/9285)查询。|
|rules.n.locationId|是|String|转发路径实例ID，可通过接口[DescribeBmForwardRules](/document/product/386/9285)查询。|
|rules.n.url|否|String|转发路径。|
...
```
GET http://domain/v2/index.php?Action=ModifyBmForwardLocation
